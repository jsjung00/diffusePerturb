...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    self.vocab_size = 53  #self.tokenizer.vocab_size #TODO: HARDCODED NUMBER OF BINS AND VOCAB SIZE
    self.gene_vocab_size = self.config.prev_model.vocab_size
    ...
    if self.config.backbone == 'dit':
      self.backbone = models.dit.DIT(
        self.config, vocab_size=self.vocab_size, cond_vocab_size=self.gene_vocab_size)
  ...
  def forward(self, x, sigma, cond_indices):
    """Returns log score."""
    sigma = self._process_sigma(sigma)
    with torch.cuda.amp.autocast(dtype=torch.float32):
      logits = self.backbone(x, sigma, cond_indices)
  ...
  def _compute_loss(self, batch, prefix):
    x0 = batch['expr'].long()
    cond_indices = batch['gene']
    attention_mask = (x0 >= 0).float().cuda() # only consider non-padding, non special tokens 

    # hack to make my token indices non-negative
    x0 = x0 + torch.abs(x0.min())

    losses = self._loss(x0, cond_indices, attention_mask)
    loss = losses.loss
  ...
  def _ddpm_caching_update(self, x, t, dt, p_x0=None, cond_vec=None):
    ...
    if p_x0 is None:
      p_x0 = self.forward(x, sigma_t, cond_vec).exp()
  ...
  def _ar_sampler(self, bsz):
    raise ValueError("diff sampler please")
  ...
  @torch.no_grad()
  def _cond_sample(self, cond_vec, inpaint_x=None, evidence_mask=None, num_steps=None, eps=1e-5, ret_inter=True):
    """Generate samples from the model condtional on input.
    inpaint_mask: indices to replace with the inpaint_x data
    ...
    if evidence_mask is not None:
      x = torch.where(evidence_mask, inpaint_x, x)
    ...
    for i in range(num_steps):
      ...
      if self.sampler == 'ddpm':
        ...
        p_x0_cache, x_next = self._ddpm_caching_update(
          x, t, dt, p_x0=p_x0_cache, cond_vec=cond_vec)
      ...
      # replace inpainting tokens again 
      if evidence_mask is not None:
        x = torch.where(evidence_mask, inpaint_x, x)
    ...
    if self.config.sampling.noise_removal:
      ...
      if self.sampler == 'analytic':
        ...
        x = self.forward(x, unet_conditioning, cond_vec).argmax(dim=-1)
  ...
  def _forward_pass_diffusion(self, x0, cond_indices):
    ...
    model_output = self.forward(xt, unet_conditioning, cond_indices)
  ...
  def _loss(self, x0, cond_indices, attention_mask):
    ...
    if self.parameterization == 'ar':
      ...
      loss = self._forward_pass_diffusion(input_tokens,cond_indices)
    
    if attention_mask is None:
      attention_mask = torch.ones_like(loss).cuda()
  ...
  def sample_subs_guidance(
    ...
    raise ValueError("Can we do easier sampling")
